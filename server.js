...
app.get('/api/debug-tournaments', function(req, res) {
  ...
  var promises = days.map(function(d) {
    ...
    return footApi('/api/matches/' + day + '/' + month + '/' + year)
      .then(function(data) {
        return { date: day+'/'+month+'/'+year, events: data && data.events ? data.events : [] };
      });
  });
  Promise.all(promises).then(function(results) {
    ...
    res.json({ total: list.length, days_scanned: days.length, tournaments: list });
  });
});
