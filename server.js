...
app.get('/api/test-betsapi', function(req, res) {
  betsApi('/v1/bet365/upcoming?sport_id=1&page=1')
    .then(function(data) {
      var results = data.results || [];
      res.json({
        total: results.length,
        pager: data.pager || {},
        sample: results.slice(0, 5).map(function(e) {
          return {
            id: e.id,
...
app.get('/api/matches', function(req, res) {
  ...
  var schedPromise = Promise.all([fetchDay(0), fetchDay(1), fetchDay(2), fetchDay(3), fetchDay(4), fetchDay(5), fetchDay(6)])
