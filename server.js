...
// Leagues to fetch every day
var LEAGUES = [
  { league_code: 'premier-league', country_code: 'england', name: 'Premier League' },
  { league_code: 'la-liga', country_code: 'spain', name: 'La Liga' },
  { league_code: 'bundesliga', country_code: 'germany', name: 'Bundesliga' },
  { league_code: 'serie-a', country_code: 'italy', name: 'Serie A' },
  { league_code: 'ligue-1', country_code: 'france', name: 'Ligue 1' },
  { league_code: 'champions-league', country_code: 'europe', name: 'Champions League' },
  { league_code: 'europa-league', country_code: 'europe', name: 'Europa League' }
];
...
app.get('/api/matches', function(req, res) {
  ...
  var promises = LEAGUES.map(function(league) {
    var url = 'https://' + LIVESCORE_HOST + '/soccer/matches-by-date?date=' + dateStr + '&league_code=' + league.league_code + '&timezone_utc=0%3A00&country_code=' + league.country_code;
    return fetch(url, {
      method: 'GET',
      headers: {
  ...
  Promise.all(promises).then(function(results) {
    ...
    res.json({ matches: allMatches, date: dateStr });
  });
});
