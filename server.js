...
// Version pour BetsAPI qui passe le nom de ligue directement
function isAllowedLeague(leagueName) {
  if (!leagueName) return false;
  var name = leagueName.toLowerCase().trim();
  for (var i = 0; i < WHITELIST.length; i++) {
    if (name === WHITELIST[i]) return true;
  }
  return false;
...
app.get('/api/test-betsapi', function(req, res) {
  ...
  betsApi('/v1/bet365/upcoming?sport_id=1&page=1').then(function(first) {
    ...
    var allPromises = [Promise.resolve(first.results || [])];
    for (var p = 2; p <= totalPages; p++) {
      allPromises.push(betsApi('/v1/bet365/upcoming?sport_id=1&page=' + p).then(function(d) { return d.results || []; }));
    }
    Promise.all(allPromises).then(function(results) {
    var all = [];
    results.forEach(function(r) { all = all.concat(r); });
    // Filtrer uniquement les ligues de notre whitelist
...
app.get('/api/matches', function(req, res) {
  ...
  var schedPromise = fetchBetsPage(1).then(function(first) {
    ...
    totalPages = Math.min(totalPages, 22); // max 22 pages
