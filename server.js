      ...
      .then(function(data) {
        ...
        return data.filter(function(e) {
          ...
          return {
            ...
            dateLabel: d.toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short', timeZone: 'Europe/Paris' })
          };
        });
      }).catch(function() { return []; });
...
app.post('/api/analyze', function(req, res) {
  ...
  Promise.all([getTeamRecent(m.homeId), getTeamRecent(m.awayId), getH2H(m.id)]).then(function(results) {
    ...
    var prompts = {
      ...
      compo: header+'ANALYSE TACTIQUE — Utilise le web search pour trouver les vraies compositions et formations:\n\n1. SYSTÈME '+m.home+' — recherche la formation habituelle et les joueurs titulaires probables. Joueurs clés, style de jeu, pressing\n2. SYSTÈME '+m.away+' — idem\n3. FORCES/FAIBLESSES — tableau HTML: Secteur|'+m.home+'|'+m.away+'|Avantage\n4. DUEL CLÉ — le matchup individuel le plus important\n5. PRÉDICTION — score exact + % confiance + raisonnement basé sur les vraies données\n\n'+TABLE+'\n'+L,
