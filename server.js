...
app.get('/api/test-betsapi', function(req, res) {
  ...
  // Construire toutes les pages d'abord
  var pageNums = [];
  for (var i = 1; i <= 22; i++) pageNums.push(i);

  Promise.all(pageNums.map(function(p) {
    ...
    res.json({
      ...
      sample: real.slice(0, 5).map(function(e) {
        return {
          ...
          time: new Date(parseInt(e.time) * 1000).toLocaleString('fr-FR')
        };
      })
    });
  });
