...
// ── FILTRE DES TOURNOIS ───────────────────────────────────────────────────
// Filtrage par nom EXACT de tournoi (uniqueTournament.name)
...
function isAllowedTournament(e) {
  ...
  var name = '';
  // Priorité au uniqueTournament.name
  if (e.tournament.uniqueTournament && e.tournament.uniqueTournament.name) {
    name = e.tournament.uniqueTournament.name;
  } else if (e.tournament.name) {
    name = e.tournament.name;
  }
  if (!name) return false;
  for (var i = 0; i < ALLOWED_NAMES.length; i++) {
    if (name === ALLOWED_NAMES[i]) return true;
  }
  return false;
}
